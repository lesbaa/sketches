...
void main() {
  float noise = snoise2(vec2(gl_FragCoord.x / 100.0, 0.0));

  float offset = -uTransitionProgress;
  float num = -1.0;
  ...
  vec4 originalColor = texture2D(uSampler, vec2(vTextureCoord.x + offset, vTextureCoord.y));
